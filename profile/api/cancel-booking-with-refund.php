...
if (!isset($_SESSION['user_id'])) {
    ...
    echo json_encode([
        'success' => false,
        'message' => __('auth.login_required') ?: 'Vui lòng đăng nhập để thực hiện thao tác này'
    ]);
    exit;
}
...
try {
    ...
    if (!$booking_id || $booking_id <= 0) {
        throw new Exception(__('profile_bookings.invalid_booking_id') ?: 'Booking ID không hợp lệ');
    }
    ...
    if (!$booking) {
        throw new Exception(__('profile_bookings.no_permission') ?: 'Không tìm thấy đặt phòng hoặc bạn không có quyền hủy');
    }

    // Check if booking can be cancelled
    if (in_array($booking['status'], ['cancelled', 'checked_out', 'no_show'])) {
        throw new Exception(__('profile_bookings.cannot_cancel_status') ?: 'Không thể hủy đặt phòng này');
    }

    if ($booking['status'] === 'checked_in') {
        throw new Exception(__('refund.already_checkedin'));
    }
    ...
    try {
        ...
        $stmt->execute([
            $booking_id,
            $_SESSION['user_id'],
            __('profile_bookings.client_cancelled') ?: 'Khách hàng hủy đặt phòng' . ' - ' . $refund_info['policy_message']
        ]);
        ...
        if ($refund_info['refund_amount'] > 0) {
            $response['message'] .= '. ' . __('refund.refund_amount_msg') . ' ' . number_format($refund_info['refund_amount']) . ' ' . __('common.currency');
            $response['refund_id'] = $refund_id ?? null;
        } else {
            $response['message'] .= '. ' . __('refund.no_refund_policy');
        }
