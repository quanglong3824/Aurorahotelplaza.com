...
<style>
    .settings-tab { padding: 10px 20px; border-bottom: 2px solid transparent; font-size:13px;
                    font-weight:600; color:#94a3b8; cursor:pointer; transition:.2s; white-space:nowrap; }
    .settings-tab.active  { color:#d4af37; border-bottom-color:#d4af37; }
    .settings-card { background:#fff; border:1px solid #e2e8f0; border-radius:16px; }
    .dark .settings-card  { background:#0f172a; border-color:#1e293b; }
    .qr-item { transition:.15s; }
    .qr-item:hover { background:#fefce8; }
    .dark .qr-item:hover { background:rgba(212,175,55,.06); }
</style>
...
<div id="tab-quick-replies">
    <div class="flex items-center justify-between mb-4">
        ...
        <button onclick="openQRModal(null)"
                class="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-400 to-amber-600
    ...
    <div class="flex gap-2 mb-4 flex-wrap" id="qrCategoryFilter">
        ...
        <button onclick="filterQR('<?php echo htmlspecialchars($cat); ?>', this)"
    ...
    <div class="settings-card overflow-hidden">
        <table class="w-full text-sm" id="qrTable">
            <thead class="bg-gray-50 dark:bg-slate-800/50">
                <tr>
                    <th class="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Tiêu đề / Shortcut</th>
                    <th class="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Nội dung</th>
                    <th class="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider w-28">Danh mục</th>
                    <th class="text-left px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider w-20">Trạng thái</th>
            ...
            <tbody id="qrTableBody">
                ...
                <tr class="qr-item border-t border-gray-100 dark:border-slate-800"
                    ...
                    <td class="px-4 py-3">
                        <div class="font-semibold text-gray-900 dark:text-white"><?php echo htmlspecialchars($r['title']); ?></div>
                        <?php if ($r['shortcut']): ?>
                    ...
                    <td class="px-4 py-3">
                        <span class="text-xs px-2 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">
                            <?php echo htmlspecialchars($r['category'] ?? 'Chung'); ?>
                        </span>
                    </td>
                    <td class="px-4 py-3">
                        <span class="inline-flex items-center gap-1 text-xs font-bold <?php echo $r['is_active'] ? 'text-green-500' : 'text-gray-400'; ?>">
                            <span class="w-2 h-2 rounded-full <?php echo $r['is_active'] ? 'bg-green-500' : 'bg-gray-400'; ?>"></span>
                            <?php echo $r['is_active'] ? 'Bật' : 'Tắt'; ?>
                        </span>
                    </td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-1">
                            <button onclick="openQRModal(<?php echo htmlspecialchars(json_encode($r)); ?>)"
                                    class="p-1.5 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/20
                                           text-amber-600 transition-colors" title="Sửa">
                                <span class="material-symbols-outlined text-sm">edit</span>
                            </button>
                            <button onclick="deleteQR(<?php echo $r['reply_id']; ?>, this)"
                                    class="p-1.5 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/20
...
<div id="tab-general" class="hidden">
    <form id="generalSettingsForm" onsubmit="saveSettings(event)">
        <div class="settings-card p-6 mb-4">
            <h4 class="font-bold text-gray-900 dark:text-white mb-5 pb-3 border-b border-gray-100 dark:border-slate-800">
                Cài đặt chung
            </h4>
            <div class="space-y-5">
                <!-- Chat enabled -->
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="chat_enabled" class="sr-only peer"
                               <?php echo $settings['chat_enabled'] === '1' ? 'checked' : ''; ?>>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="autoReplyToggle" name="auto_reply_enabled" class="sr-only peer"
                               <?php echo $settings['auto_reply_enabled'] === '1' ? 'checked' : ''; ?>
                ...
                <div id="autoReplyMsgBox" class="<?php echo $settings['auto_reply_enabled'] !== '1' ? 'hidden' : ''; ?>">
                    ...
                    ><?php echo htmlspecialchars($settings['auto_reply_message']); ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="offline_message" rows="2"
                              class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700
                                     bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-200
                                     text-sm focus:outline-none focus:border-amber-400 transition-colors"
                    ><?php echo htmlspecialchars($settings['offline_message']); ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="max_conversations" min="1" max="50"
                           value="<?php echo (int)$settings['max_conversations']; ?>"
                           class="w-24 px-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700
                                  bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-200
        ...
        <div class="flex justify-end">
            <button type="submit"
                    class="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-amber-400 to-amber-600
    ...
    <form id="timingSettingsForm" onsubmit="saveSettings(event)">
        ...
        <div class="settings-card p-6 mb-4">
            <h4 class="font-bold text-gray-900 dark:text-white mb-2">
                ...
                <span class="text-xs font-normal text-gray-400 ml-2">(khuyến nghị: global 3s, conv 2s trên shared hosting)</span>
            </h4>
            <div class="grid grid-cols-2 gap-4 mt-4">
                <div>
                    ...
                    <input type="number" name="sse_interval_global" min="1" max="10"
                           value="<?php echo (int)$settings['sse_interval_global']; ?>"
                           class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700
                                  bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-200
                                  text-sm focus:outline-none focus:border-amber-400">
                ...
                <div>
                    ...
                    <input type="number" name="sse_interval_conv" min="1" max="10"
                           value="<?php echo (int)$settings['sse_interval_conv']; ?>"
                           class="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-slate-700
                                  bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-200
                                  text-sm focus:outline-none focus:border-amber-400">
...
<div id="qrModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4"
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-slate-700">
        ...
        <form id="qrForm" onsubmit="saveQR(event)" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-500 mb-1">Tiêu đề <span class="text-red-500">*</span></label>
                <input type="text" id="qrTitle" required placeholder="VD: Chào mừng khách"
            ...
            <div>
                <label class="block text-xs font-bold text-gray-500 mb-1">Shortcut <span class="text-gray-400">(không dấu, không space)</span></label>
                <div class="flex items-center gap-1">
                    <span class="text-amber-500 font-bold text-lg">/</span>
                    <input type="text" id="qrShortcut" placeholder="chao"
                           pattern="[a-z0-9_-]*"
                           class="flex-1 px-3 py-2.5 rounded-xl border border-gray-200 dark:border-slate-700
                                  bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-200
                                  text-sm focus:outline-none focus:border-amber-400 transition-colors">
                </div>
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-500 mb-1">Nội dung <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="text" id="qrCategory" placeholder="VD: Chào hỏi, Giá phòng..."
                           class="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-slate-700
                                  bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-200
                                  text-sm focus:outline-none focus:border-amber-400 transition-colors">
                </div>
                <div>
                    ...
                    <input type="number" id="qrSortOrder" value="0" min="0"
                           class="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-slate-700
                                  bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-200
                                  text-sm focus:outline-none focus:border-amber-400 transition-colors">
                </div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="button" onclick="closeQRModal()"
                        class="flex-1 px-4 py-2.5 border border-gray-200 dark:border-slate-700
                ...
                <button type="submit"
                        class="flex-1 px-4 py-2.5 bg-gradient-to-r from-amber-400 to-amber-600
...
async function saveQR(e) {
    ...
    const r = await fetch('/admin/api/manage-quick-replies.php', {
        method: 'POST',
...
async function saveSettings(e) {
    ...
    const r = await fetch('/admin/api/chat-settings.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
