...
try {
    ...
    $stmtAll = $db->query("SELECT SUM(total_hits) as hits, SUM(unique_visitors) as unique_v, SUM(mobile_hits) as mobile, SUM(desktop_hits) as desktop, SUM(bot_hits) as bot FROM traffic_stats_daily");
    $all = $stmtAll->fetch();
    if ($all) {
    ...
    try {
        $db->exec("CREATE TABLE IF NOT EXISTS traffic_logs (id INT AUTO_INCREMENT PRIMARY KEY, session_id VARCHAR(100) NOT NULL, ip_address VARCHAR(45) NOT NULL, user_id INT DEFAULT NULL, page_url TEXT NOT NULL, page_title VARCHAR(255) DEFAULT NULL, referer TEXT DEFAULT NULL, user_agent TEXT DEFAULT NULL, device_type ENUM('desktop', 'mobile', 'tablet', 'bot') DEFAULT 'desktop', is_unique TINYINT(1) DEFAULT 0, bot_type ENUM('good', 'bad', 'none') DEFAULT NULL, bot_name VARCHAR(255) DEFAULT NULL, visit_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP, duration INT DEFAULT 0, KEY idx_session (session_id), KEY idx_visit_time (visit_time), KEY idx_ip (ip_address)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;");
        $db->exec("CREATE TABLE IF NOT EXISTS traffic_stats_daily (id INT AUTO_INCREMENT PRIMARY KEY, stat_date DATE NOT NULL UNIQUE, total_hits INT DEFAULT 0, unique_visitors INT DEFAULT 0, mobile_hits INT DEFAULT 0, desktop_hits INT DEFAULT 0, bot_hits INT DEFAULT 0, total_duration INT DEFAULT 0, KEY idx_date (stat_date)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;");
        header("Refresh:0");
    } catch (Exception $e2) {}
}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="stat-card">
            <div class="flex items-center justify-between mb-2">
                <p class="text-sm font-bold text-slate-500 uppercase tracking-wider">Tổng lượt truy cập</p>
            ...
            <p class="text-xs text-slate-500 font-bold mt-2 flex items-center gap-1">
                Bao gồm cả Human & Bot
            </p>
        </div>
        ...
        <div class="stat-card">
            ...
            <p class="text-xs text-green-600 font-bold mt-2 flex items-center gap-1">
                ...
                +<?php echo number_format($stats['unique_today']); ?> hôm nay
            </p>
        </div>

        <div class="stat-card">
            <div class="flex items-center justify-between mb-2">
                <p class="text-sm font-bold text-slate-500 uppercase tracking-wider">Phân loại Traffic</p>
                <span class="material-symbols-outlined text-emerald-500">smart_toy</span>
            </div>
            <?php 
                $human_hits = $stats['total_hits'] - $stats['bot_hits'];
                $human_pc = $stats['total_hits'] > 0 ? round(($human_hits / $stats['total_hits']) * 100) : 0;
            ?>
            <div class="flex items-end justify-between">
                <div>
                    <p class="text-3xl font-black text-slate-900 dark:text-white"><?php echo $human_pc; ?>%</p>
                    <p class="text-[10px] text-slate-400 font-bold uppercase">Người thật</p>
                </div>
                <div class="text-right">
                    <p class="text-xl font-bold text-rose-500"><?php echo 100 - $human_pc; ?>%</p>
                    <p class="text-[10px] text-slate-400 font-bold uppercase">Bot/Crawl</p>
                </div>
            </div>
            <div class="w-full h-1.5 bg-rose-100 dark:bg-rose-900/30 rounded-full mt-3 overflow-hidden flex">
                <div class="h-full bg-emerald-500" style="width: <?php echo $human_pc; ?>%"></div>
            </div>
        </div>

        <div class="stat-card">
            <div class="flex items-center justify-between mb-2">
                <p class="text-sm font-bold text-slate-500 uppercase tracking-wider">Thiết bị (Người thật)</p>
                <span class="material-symbols-outlined text-blue-500">devices</span>
            </div>
            <?php 
                $total_device_hits = $stats['mobile_hits'] + $stats['desktop_hits'];
                $mobile_pc = $total_device_hits > 0 ? round(($stats['mobile_hits'] / $total_device_hits) * 100) : 0;
            ?>
            <div class="flex items-end justify-between">
                <div>
                    <p class="text-3xl font-black text-slate-900 dark:text-white"><?php echo $mobile_pc; ?>%</p>
                    <p class="text-[10px] text-slate-400 font-bold uppercase">Mobile</p>
                </div>
                <div class="text-right">
                    <p class="text-xl font-bold text-amber-500"><?php echo 100 - $mobile_pc; ?>%</p>
                    <p class="text-[10px] text-slate-400 font-bold uppercase">Desktop</p>
                </div>
            </div>
            <div class="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full mt-3 overflow-hidden flex">
                <div class="h-full bg-blue-500" style="width: <?php echo $mobile_pc; ?>%"></div>
                <div class="h-full bg-amber-500" style="width: <?php echo 100 - $mobile_pc; ?>%"></div>
            </div>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors text-sm">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                ...
                                <?php if ($log['bot_name']): ?>
                                    <span class="text-[9px] text-rose-500 font-bold uppercase italic"><?php echo htmlspecialchars($log['bot_name']); ?></span>
                                <?php endif; ?>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <span class="material-symbols-outlined text-slate-400 align-middle text-sm" title="<?php echo $log['device_type']; ?>"><?php echo $device_icon; ?></span>
                                <?php if ($log['bot_type'] === 'good'): ?>
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500" title="Good Bot"></span>
                                <?php elseif ($log['bot_type'] === 'bad'): ?>
                                    <span class="w-1.5 h-1.5 rounded-full bg-rose-500" title="Bad Bot"></span>
                                <?php endif; ?>
                            </div>
                        </td>
                        <td class="px-6 py-4">
