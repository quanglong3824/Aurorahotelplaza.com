...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if (empty($full_name))
        $errors[] = __('auth.error_name_required');
    if (empty($email))
        $errors[] = __('auth.error_email_required');
    if (!filter_var($email, FILTER_VALIDATE_EMAIL))
        $errors[] = __('auth.error_email_invalid');
    if (empty($phone))
        $errors[] = __('auth.error_phone_required');
    if (empty($password))
        $errors[] = __('auth.error_password_required');
    if (strlen($password) < 6)
        $errors[] = __('auth.error_password_min');
    if ($password !== $confirm_password)
        $errors[] = __('auth.error_password_mismatch');
    if (!$agree_terms)
        $errors[] = __('auth.error_terms_required');
                            ...
                            <div class="success-modal-content">
                                ...
                                <div class="modal-buttons">
                                    <button class="modal-btn modal-btn-primary"
                                        onclick="window.location.href='<?php echo url('auth/login.php'); ?>'">
                                        <?php _e('auth.login_now'); ?>
                                    </button>
                                    <button class="modal-btn modal-btn-secondary"
                                        onclick="window.location.href='<?php echo url('index.php'); ?>'">
