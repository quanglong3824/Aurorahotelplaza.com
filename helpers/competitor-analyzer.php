        ...
        $system_instruction .= "\n\nYÊU CẦU PHẢN HỒI: Chỉ trả về JSON duy nhất, không kèm giải thích. Cấu trúc JSON:
{
    ...
    /**
     * Tự động khám phá các đối thủ lân cận dựa trên trí tuệ nhân tạo
     */
    public static function discoverNearbyCompetitors() {
        $db = getDB();
        
        $system_instruction = "Bạn là một chuyên gia khảo sát thị trường du lịch và khách sạn. 
Nhiệm vụ của bạn là liệt kê ít nhất 8 khách sạn hoặc căn hộ dịch vụ lớn nhất tại khu vực Biên Hòa, Đồng Nai.
YÊU CẦU PHẢN HỒI: Chỉ trả về JSON duy nhất. Cấu trúc JSON:
{
    \"discovered\": [
        { \"name\": \"Tên khách sạn\", \"url\": \"URL website chính thức\" }
    ]
}";

        $prompt = "Hãy liệt kê các đối thủ cạnh tranh chính của Aurora Hotel Plaza tại Biên Hòa.";
        ...
        try {
            ...
            foreach ($data['discovered'] as $item) {
                // Kiểm tra xem đã tồn tại URL này chưa để tránh trùng lặp
                ...
                if (!$check->fetch()) {
                    ...
                    $stmt->execute([$item['name'], $item['url'], 'Tự động khám phá bởi AI. Hãy phân tích tổng quan dịch vụ của họ.']);
                    $new_count++;
                }
            }
            return $new_count;
